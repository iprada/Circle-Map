                    ...
                    if (read.is_reverse == True and supl_info[2] == '-') or (
                        ...
                        if read.reference_start > int(supl_info[1]):
                            ...
                            mate_interval = [interval.chrom, int(supl_info[1]) - (ref_alignment_length),
                                             (int(supl_info[1]) + (ref_alignment_length)), "SA", "L",int(supl_info[4])]
                        ...
                        # SA is upstream, the interval is end - read length, end
                            ...
                            mate_interval = [interval.chrom, (int(supl_info[1]) - (ref_alignment_length)),
                                             int(supl_info[1]) + (ref_alignment_length), "SA", "R",int(supl_info[4])]
...
def get_realignment_intervals(bed_prior,interval_extension):
    ...
    if np.any(read_types == 'SC') == False:
        ...
        candidate_mates = bt.BedTool.from_dataframe(candidate_mates_dataframe).sort().merge(c=[1,5],o=['count','distinct'])
        ...
        candidate_mates = bt.BedTool.from_dataframe(candidate_mates_no_sc).sort().merge(c=[1,5],o=['count','distinct'])
        print(candidate_mates)
...
def add_discordants(sa_sc_dict,discordants):
    ...
    disc_bed_grouped_pandas = pd.DataFrame.from_records(discordants, columns=labels)
    sorted_grouped = disc_bed_grouped_pandas.groupby(['chrom', 'start', 'end'], sort=False).read.nunique()
    sorted_grouped_list = [x for x in sorted_grouped.reset_index().values.tolist()]
    sc_bed_grouped = bt.BedTool(sorted_grouped_list)

    number_of_discordants = len(sc_bed_grouped)
